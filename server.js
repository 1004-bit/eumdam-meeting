...
function currentUser(req) {
  const token = parseCookies(req).sai_session;
...
function createSession(res, userId) {
  ...
  return { 'Set-Cookie': `sai_session=${token}; Path=/; HttpOnly; SameSite=Lax; Max-Age=2592000` };
}
...
async function api(req, res, url) {
  ...
  if (method === 'POST' && url.pathname === '/api/logout') {
    ...
    return json(res, 200, { ok: true }, { 'Set-Cookie': 'sai_session=; Path=/; HttpOnly; SameSite=Lax; Max-Age=0' });
  }
...
server.listen(PORT, HOST, () => console.log(`SAI community listening on http://${HOST}:${PORT}`));
